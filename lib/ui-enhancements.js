(function () {
  ...
  function renderSolution() {
    ...
    container.innerHTML = `
      ...
      ${solution.level === 'pattern-level' ? '<div class="settings-note">This entry currently uses a pattern-level explanation. It is still self-contained for study, and Interview Lab will only score it automatically when a dedicated local judge is available.</div>' : ''}
    `;
  }
  ...
  function enhanceDescription() {
    const copy = $('.problem-copy');
    if (!copy || copy.dataset.selfContained === 'true') return;
    const problem = currentProblem();
    if (!problem) return;

    $$('.source-card', copy).forEach((card) => {
      if (/Canonical source/i.test(card.textContent || '')) card.remove();
    });

    const details = document.createElement('div');
    details.className = 'self-contained-details';
    details.innerHTML = `
      <div class="learn-card">
        <h3>Constraints & assumptions</h3>
        <ul class="evidence-list">${(problem.constraints || []).map((item) => `<li>${escapeHtml(item)}</li>`).join('')}</ul>
      </div>
      <div class="learn-card">
        <h3>Edge cases to think through</h3>
        <ul class="evidence-list">${(problem.edgeCases || []).map((item) => `<li>${escapeHtml(item)}</li>`).join('')}</ul>
      </div>
      <div class="source-card">
        <div><strong>Interview Lab practice contract</strong><div class="muted">${escapeHtml(problem.practiceContract || 'Self-contained practice description.')}</div></div>
        <span class="badge neutral">${problem.validationCoverage === 'curated' ? 'local judge' : 'custom tests'}</span>
      </div>
      <div class="settings-note">${escapeHtml(problem.sourceNote || 'This is an original practice description written for Interview Lab.')}</div>
    `;
    copy.appendChild(details);
    copy.dataset.selfContained = 'true';
  }

  function protectHiddenCases() {
    $$('.case-chip').forEach((button) => {
      if (button.classList.contains('hidden-case-chip')) return;
      if (/^Hidden\s/i.test(button.textContent || '')) {
        button.disabled = true;
        button.classList.add('hidden-case-chip');
        button.textContent = 'Hidden test';
        button.title = 'This edge case is run automatically when you Submit.';
      }
    });
  }

  function restoreSearchFocus() {
    if (!recentSearch || Date.now() - recentSearch.at > 650) return;
    const input = $('#searchProblems');
    if (!input) return;
    input.focus({ preventScroll: true });
    const max = input.value.length;
    const start = Math.min(recentSearch.start ?? max, max);
    const end = Math.min(recentSearch.end ?? start, max);
    try { input.setSelectionRange(start, end); } catch {}
    recentSearch = null;
  }

  document.addEventListener('input', (event) => {
    if (event.target?.id !== 'searchProblems') return;
    recentSearch = {
      at: Date.now(),
